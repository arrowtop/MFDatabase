 ...
 * 
 * MFMainGenerator class is used to generator the main class to implement the query.
...
public class MFMainGenerator {
	...
	/**
	 * Constructor is used to get a path to save output file, MFStructure information
	 * and database schema.							
	 * @param path  The location of the output file -- OutputMain.java
	 * @param mfsi	MFStructure  -- Built by MFStructGenerator
	 * @param dm	DatabaseManager  -- Database's information
	 */
	MFMainGenerator(String path, MFStructInfo mfsi, DatabaseManager dm, boolean isOutputOnUI){
		...
		String fetchAllSQL =  "\t\t\tString sql = \"SELECT * FROM " + tableName + "\";\n"
								...
								+ "\t\t\tResultSet rs;\n";
		//To build while loop
		String scanLoopInfo = ScanLoopInfo(mfsi);
	 ...
	 */
	public String ScanLoopInfo(MFStructInfo mfsi){
		String scanLoopInfo ="\t\t\t//By scanning the table to get the value of attribute needed.\n";
		int loopTime = mfsi.getNumOfGv() + 1;  //Times of loop needed
		int start = 1; //the start loop
		if(mfsi.getAfCoreList().containsKey("0")){  //if loop needed for 0th loop
			start = 0;
		}
		for(int i = start; i < loopTime; i++){ 
			...
			int currentLoop = i;  //Current loop
			...
			if(condMap.containsKey(currentLoop)){
				...
				scanLoopInfo_inner = "\t\t\twhile(rs.next()) {\n"
						+ attrList + mapKey + isExistInMFStructure + aggrFunStructure +"\t\t\t\t}\n"
						+ "\n\t\t\t\tif( " + mfsi.getCondList().get(i) + " ){\n";
			}else{
				scanLoopInfo_inner = "\t\t\twhile(rs.next()) {\n"
						+ attrList + mapKey + "\n\t\t\t\tif( " + mfsi.getCondList().get(i) + " ){\n";
			}
			...
			Map<String,Stack<String>> aggrFun = mfsi.getAfCoreList().get(i + "");
			...
			while (iter2.hasNext()) { 
			    ...
			    while(!val.empty()){
			    	String attr = val.pop();
			    	mfstructSet += "\t\t\t\t\t\t" + "mfstruct.set_" + key +"_" + i + "_" + attr + "(" + attr + ");\n";
			    }
			} 
			...
			scanLoopInfo += fetchAllQuery + scanLoopInfo_inner + isKeyExist + mfstructSet + inMap + mfstructSet + res;
		}
